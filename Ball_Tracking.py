...
def calc_distancia(frame, centro_b, centro_r, xcentR, ycentR, xcentB, ycentB):
    cv2.line(frame, centro_b, centro_r, (0,0,255), 3, 8)
    euclid = np.sqrt((xcentB - xcentR)**2 + (ycentB - ycentR)**2)
    num = round(euclid, 2)
    dis = str(num)
    cv2.putText(frame, dis, centro_b, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0))
    return frame
...
def MascRobos(frame):
    ...
    return frame, center, xcentro, ycentro 
...
while not keyboard.is_pressed('q'):   # Função de captura da imagem
    ...
    mascB, centro_b, xcentR, ycentR = MascBola(frame)
    mascR, centro_r, xcentB, ycentB = MascRobos(mascB)
    dist = calc_distancia(mascR, centro_b, centro_r, xcentR, ycentR, xcentB, ycentB)
